...
As we can see from the introduction table, there're 213451 rows and 29 columns. 14.6% of observations are missing. Additionally, I find that some continous variables such as signup_flow should be factor, I will process these variables in the following part.
...
We can see that the date in our data table is composed of year, month and day. I want to see seasonal changes in registration and activation. So here I seperate year, month and day into different columns.
...
Next,I divide the target countries and devices into several categories to see the chioces of different genders and ages for these categories.
...
I extracted the top prediction for each user and created a confusion matrix for our prediction.
...
We can see that Recursive Partitioning has the highest Kappa value, while Extreme Gradient Boosting has the highest accuracy. I pick the model with the highest Kappa value (i.e. rpart) because Kappa takes into account class imbalance. As we can see from the EDA, in our training data, our class is highly imbalanced. So Kappa is a better measure here.

Then I use recursive partitioning to predict test set.
...
We can see that our prediction results don't give us much information about users' first booking destination since there're only two classes, US and NDF. This means that most of the users will first book a place in the U.S. or they will not book according to our prediction. This is because the data is highly imbalanced, and the classifiers tend to be biased towards the majority class (i.e. US and NDF) and hence perform poorly on the minority class. Besides, we can see that many of the attributes are not useful in training the model, and our raw data is intuitively not very relevant to the results what I need to predict. If the original data contains more variables, such as the user's previous hotel and ticket consumption records, our predicted results will be more accurate. 
Another reason that may affect our results is that I did not include date-related variables. After research, I found that a large number of users were not recorded for those variables. So if I use them, it will greatly reduce the size of sample I can use.
